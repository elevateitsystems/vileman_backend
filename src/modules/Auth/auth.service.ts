...
export class AuthService extends BaseService<User> {
  ...
  async login(data: LoginInput): Promise<AuthResponse> {
    ...
    // ✅ Handle guest cart merging if guestId is provided
    if (data.guestId) {
      await this.mergeCart(data.guestId, user.id);
    }
   ...
   */
  private async mergeCart(guestId: string, userId: string): Promise<void> {
    try {
      ...
      AppLogger.info("Guest cart merged successfully", { guestId, userId });
    } catch (error) {
      AppLogger.error("Failed to merge guest cart", {
        guestId,
        userId,
        error: error instanceof Error ? error.message : "Unknown error",
      });
      // Don't throw error here to allow login to proceed
    }
  }
